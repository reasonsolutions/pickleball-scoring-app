...
export default function SideDisplay() {
  ...
  const sideCourtMatchesRef = useRef([]);
  ...
  const cleanupStaleTimeouts = useCallback(async () => {
    ...
    try {
      console.log('🧹 Side Court - Cleaning up stale timeout data...');
      ...
      snapshot.docs.forEach(docSnapshot => {
        ...
        if (data.timeoutInProgress && data.timeoutInProgress !== null && !isLive) {
          console.log(`🧹 Side Court - Clearing stale timeout for non-live match: ${data.team1Name} vs ${data.team2Name} (status: ${data.status})`);
      ...
      if (cleanupPromises.length > 0) {
        ...
        console.log(`🧹 Side Court - Cleaned up ${cleanupPromises.length} stale timeouts`);
      } else {
        console.log('🧹 Side Court - No stale timeouts found');
      }
    } catch (error) {
      console.error('Side Court - Error cleaning up stale timeouts:', error);
    }
  }, [tournamentId]);

  // Stable increment function using ref
  const incrementFirebaseReads = useCallback((operation, details = '') => {
    console.log(`🔥 Side Court Firebase Read: ${operation} ${details}`);
  ...
  const extractVideoId = useCallback((url) => {
    ...
    console.log('🎥 Side Court - Extracting video ID from URL:', url);
    
    // Handle YouTube Live URLs (e.g., https://www.youtube.com/live/VIDEO_ID)
    const liveMatch = url.match(/youtube\.com\/live\/([a-zA-Z0-9_-]{11})/);
    if (liveMatch) {
      console.log('🎥 Side Court - Found YouTube Live ID:', liveMatch[1]);
      return liveMatch[1];
    }
    ...
    console.log('🎥 Side Court - Extracted video ID:', videoId);
  ...
  const loadMediaItems = useCallback(async () => {
    ...
    try {
      console.log('🔍 Side Court - Loading media items for tournament:', tournamentId);
      const mediaDoc = await getDoc(doc(db, 'video_links', tournamentId));
      incrementFirebaseReads('Side Court Media Items Read', tournamentId);
      ...
      if (mediaDoc.exists()) {
        ...
        console.log('🎬 Side Court - Media items loaded successfully:', {
        ...
        console.log('🎬 Side Court - No media items document found for tournament:', tournamentId);
        setMediaItems([]);
      }
    } catch (error) {
      console.error('❌ Side Court - Error loading media items:', error);
      setMediaItems([]);
    }
  ...
  const checkShouldShowFeaturedImage = useCallback((matches) => {
    console.log('🖼️ Side Court - Featured Image Check - Starting:', {
      totalMatches: matches.length,
      matchStatuses: matches.map(m => ({ id: m.id, status: m.status, teams: `${m.team1Name} vs ${m.team2Name}` }))
    });

    if (matches.length === 0) {
      console.log('🖼️ Side Court - No matches found - not showing featured image');
    ...
    console.log('🖼️ Side Court - Featured Image Decision:', {
      allScheduled,
  ...
  const checkShouldShowAdsMedia = useCallback((matches, currentLiveMatch) => {
    console.log('🎬 Side Court - Ads Media Check - Starting:', {
      ...
      totalMatches: matches.length,
      currentShowAdsMedia: showAdsMedia,
      currentTimeoutStarted: timeoutStarted,
      currentIsTimeoutActive: isTimeoutActive
    });

    if (mediaItems.length === 0) {
      console.log('🎬 Side Court - No media items available - not showing ads');
      setShowAdsMedia(false);
      return;
    }

    // Check if ANY LIVE match has timeout in progress
    const timeoutMatches = matches.filter(match => {
      ...
      console.log('🔍 Side Court - Timeout check for match:', {
        matchId: match.id,
    ...
    const hasTimeoutInProgress = timeoutMatches.length > 0;
    ...
    console.log('🎬 Side Court - Ads Media Decision:', {
      hasTimeoutInProgress,
      hasCompletedMatch,
      hasLiveMatch,
      timeoutMatchesCount: timeoutMatches.length,
      timeoutMatches: timeoutMatches.map(m => ({
        id: m.id,
      ...
      shouldShow,
      mediaItemsCount: mediaItems.length,
      previousShowAdsMedia: showAdsMedia,
      willChangeState: shouldShow !== showAdsMedia
    });

    // Force state update even if the value is the same to ensure consistency
    if (shouldShow !== showAdsMedia) {
      console.log('🎬 Side Court - Changing ads media state from', showAdsMedia, 'to', shouldShow);
    ...
    if (hasTimeoutInProgress && shouldShow && !timeoutStarted) {
      console.log('⏰ Side Court - Starting timeout timer - 60 seconds', {
        hasTimeoutInProgress,
        shouldShow,
        timeoutStarted,
        timeoutMatchesCount: timeoutMatches.length
      ...
      console.log('⏰ Side Court - Timeout ended, resetting timer', {
        hasTimeoutInProgress,
        timeoutStarted,
        timeoutMatchesCount: timeoutMatches.length
      });
    ...
    } else if (hasTimeoutInProgress && !shouldShow) {
      console.log('⚠️ Side Court - Timeout detected but ads not showing - possible issue:', {
        hasTimeoutInProgress,
        shouldShow,
        mediaItemsCount: mediaItems.length,
        timeoutMatchesCount: timeoutMatches.length
  ...
  }, [mediaItems.length, timeoutStarted, showAdsMedia, isTimeoutActive]);
  ...
  const updateMatchesForFixture = useCallback((selectedFixture, matches) => {
    console.log('=== UPDATING SIDE COURT MATCHES FOR FIXTURE ===');
    ...
    console.log('Side court fixture matches found:', fixtureMatches.length);
    ...
    console.log('Side court match categorization:', {
    ...
    console.log('Side Court Fixture Score:', `${fixtureScore.team1Wins} - ${fixtureScore.team2Wins}`);
    ...
    if (fixtureScore.team1Wins === 3 && fixtureScore.team2Wins === 3) {
      ...
      if (gameBreakerMatch) {
        ...
        console.log('🎯 SIDE COURT GAME BREAKER MATCH SET (3-3 scenario):', liveMatchToSet.team1Name, 'vs', liveMatchToSet.team2Name);
      }
    }
    ...
    if (!liveMatchToSet) {
      if (inProgress.length > 0) {
        ...
        console.log('✅ SIDE COURT LIVE MATCH SET (being umpired):', liveMatchToSet.team1Name, 'vs', liveMatchToSet.team2Name);
      } else if (upcoming.length > 0) {
        liveMatchToSet = upcoming[0];
        console.log('⏳ SIDE COURT LIVE MATCH SET (upcoming):', liveMatchToSet.team1Name, 'vs', liveMatchToSet.team2Name);
      } else if (completed.length > 0) {
        liveMatchToSet = completed[completed.length - 1];
        console.log('✅ SIDE COURT LIVE MATCH SET (last completed):', liveMatchToSet.team1Name, 'vs', liveMatchToSet.team2Name);
      }
    }
  ...
  }, [tournamentId, incrementFirebaseReads]); // Only depends on tournamentId
  ...
  useEffect(() => {
    ...
    console.log('🎥 Side Court - Setting up DRS video listener for match:', liveMatch.id);

    const unsubscribe = onSnapshot(doc(db, 'fixtures', liveMatch.id), (docSnapshot) => {
      if (docSnapshot.exists()) {
        const matchData = docSnapshot.data();
        
        console.log('🎥 Side Court - DRS Video data received:', {
          matchId: liveMatch.id,
          drsVideoActive: matchData.drsVideoActive,
          drsVideoUrl: matchData.drsVideoUrl,
          isDRSVideoPlaying,
        ...
        // Check if DRS video should be stopped
          ...
          console.log('🎥 Side Court - DRS Video stopped for match:', {
            matchId: liveMatch.id,
            teams: `${matchData.team1Name} vs ${matchData.team2Name}`,
            court: matchData.court
          });
        ...
        console.log('🎥 Side Court - Match document does not exist:', liveMatch.id);
      }
    });
  ...
  useEffect(() => {
    if (isTimeoutActive && timeoutTimer !== null && timeoutTimer > 0) {
      console.log('⏰ Side Court - Starting timer countdown interval, current timer:', timeoutTimer);
      ...
      timeoutIntervalRef.current = setInterval(() => {
        setTimeoutTimer(prev => {
          console.log('⏰ Side Court - Timer tick, current value:', prev);
          if (prev <= 1) {
            clearInterval(timeoutIntervalRef.current);
            timeoutIntervalRef.current = null;
            setIsTimeoutActive(false);
            ...
            console.log('⏰ Side Court - Timeout timer ended');
            ...
            if (liveMatch?.id) {
              updateDoc(doc(db, 'fixtures', liveMatch.id), {
                ...
                console.log('✅ Side Court - Timeout cleared in Firebase');
              }).catch(error => {
                console.error('❌ Side Court - Error clearing timeout in Firebase:', error);
              });
            }
  ...
  }, [isTimeoutActive, timeoutTimer, liveMatch?.id]); // Only run when these change
  ...
  useEffect(() => {
    ...
    console.log('🎬 Side Court - Media cycling enabled for', mediaItems.length, 'items');
  }, [showAdsMedia, mediaItems.length]);

  // Handle media cycling - videos end naturally, images need timer
  const handleMediaEnd = useCallback(() => {
    if (mediaItems.length > 1) {
      console.log('🎬 Side Court - Media ended, switching to next item');
  ...
  useEffect(() => {
    ...
    if (showAdsMedia && currentItem && currentItem.type === 'image') {
      console.log('🖼️ Side Court - Starting 10-second timer for image:', currentItem.title);
      const timer = setTimeout(() => {
        console.log('🖼️ Side Court - Image timer ended, switching to next media');
        handleMediaEnd();
      }, 10000); // 10 seconds
  ...
  useEffect(() => {
    ...
    console.log('🔧 Setting up Firebase listeners for Side Display (ONE TIME)');
    ...
    const displaySettingsUnsubscribe = onSnapshot(
      ...
      (docSnapshot) => {
        incrementFirebaseReads('Side Display Settings Listener', displaySettingId);
        
        if (docSnapshot.exists()) {
          const displaySetting = docSnapshot.data();
          const selectedFixture = displaySetting.currentSideFixture || displaySetting.currentFixture;
          
          if (selectedFixture) {
            console.log('📺 Side display settings updated:', selectedFixture);
            currentFixtureRef.current = selectedFixture;
            setCurrentFixture(selectedFixture);
            
            // Update matches with current fixture
            if (sideCourtMatchesRef.current.length > 0) {
              updateMatchesForFixture(selectedFixture, sideCourtMatchesRef.current);
            }
          }
        }
      },
      (error) => {
        console.error('Error listening to side display settings:', error);
      }
    );
    ...
    const matchesUnsubscribe = onSnapshot(
      matchesQuery,
      (snapshot) => {
        incrementFirebaseReads('Side Court Matches Query', `${snapshot.docs.length} total matches`);
        ...
        // Filter for side court matches on this date (NOT center court)
        ...
        console.log('🔥 Side court matches updated:', dateMatches.length);
        
        // Store in ref
        sideCourtMatchesRef.current = dateMatches;
      ...
      (error) => {
        console.error('Error in side court matches listener:', error);
      }
    );

    // Cleanup function
    return () => {
      console.log('🧹 Cleaning up Side Display Firebase listeners');
          ...
          {(() => {
            ...
            return (
              <>
                ...
                <div className="w-full h-full flex items-center justify-center">
                  {(() => {
                    console.log('🎬 Side Court - Rendering media overlay:', {
                      currentItem,
                    ...
                    if (currentItem.type === 'image') {
                      // Image display
                      console.log('🖼️ Side Court - Displaying image:', currentItem.url);
                      ...
                      if (videoId) {
                        ...
                        console.log('🎬 Side Court - YouTube embed URL:', embedUrl);
                        ...
                        console.log('🎬 Side Court - Direct video URL:', currentItem.url);
                        ...
                        console.log('🎬 Side Court - Invalid video format:', currentItem.url);
                        return (
                          <div className="text-center text-white">
                            <div className="text-8xl mb-8">🎥</div>
                      ...
                      console.log('🎬 Side Court - Unknown media type:', currentItem.type);
                      return (
                        <div className="text-center text-white">
                          <div className="text-8xl mb-8">❓</div>
